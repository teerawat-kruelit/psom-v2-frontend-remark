...
import { Department, PaginationState } from '@/types'
...
export const useDepartmentManagementStore = create<DepartmentState>((set, get) => ({
  dataTable: [],
  isLoading: false,
  fetchDepartments: async () => {
    ...
    try {
      const response = await GET<Department>('/central/department-management/search', {
        params: {
      ...
      const payload = response.data as { count: number; rows: Department[] }
