...
export const useBranchManagementStore = create<BranchState>((set, get) => ({
  dataTable: [],
  isLoading: false,
  fetchBranches: async () => {
    set({ isLoading: true })
    ...
    try {
      const response = await GET<Branch>('/central/branch-management/search', {
        params: {
      ...
      const payload = response.data as { count: number; rows: Branch[] }
